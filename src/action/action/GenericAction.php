...
abstract class GenericAction {
    ...
    public function get($id = null, string $getFunc = 'getData') {
        ...
        return $this->process($result, $getFunc);
    }
     ...
     * @throws Exception Cuando no cumple con la validación
     */
    public function update($entity, string $getFunc = 'getData') {
        
        //recoge id
        $this->serviceValidation->checkToUpdate($entity);
        
        //funciona igual que guardar, doctrine tiene su propio esquema interno
        
        //guarda entidad bbdd
        $result = $this->serviceDao->save($entity);
        
        //comprueba que ha de devolver entidades
        if ($getFunc === null){
            return $result;
        }
    ...
    private function process($entityOrList, string $getFunc) {
